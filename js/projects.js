...
(function () {
  ...
  function card(p) {
    const cover = p.cover
      ? `<img src="${escape(p.cover)}" alt="Cover image for ${escape(
          p.title
        )}" loading="lazy" decoding="async">`
      : `<div aria-hidden="true" class="skeleton"></div>`;

    const tags = (p.tags || [])
      .map((t) => `<span class="tag" aria-label="tag">${escape(t)}</span>`)
      .join("");
...
<article class="project-card" data-tags="${escape((p.tags || []).join(","))}">
  ...
  <div class="project-meta">
    <span class="project-year" aria-label="year">${p.year ?? ""}</span>
  ...
  async function init() {
    ...
    try {
      // adjust path if you move projects.json (e.g. "data/projects.json")
